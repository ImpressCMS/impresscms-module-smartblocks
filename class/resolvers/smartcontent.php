...
class SmartContentResolver extends SmartblocksResolver {
    var	$locations;
    var	$pages;

    function SmartContentResolver() {
        $this->pages=array();
    }
    ...
    function resolveLocation() {
        if(strstr($_SERVER['SCRIPT_NAME'],"page.php")) {
            $page_handler = xoops_getmodulehandler('page', 'smartcontent');
            $page = $page_handler->get($_REQUEST['pageid']);
            return $page->getVar('parentid');
        }
    ...
    function getLocationName($location) {
        $cat_handler =& xoops_getmodulehandler('category', 'smartcontent');
